...
# 检测 Tailscale IP（稳定，不随重启变化）
TAILSCALE_BIN="/Applications/Tailscale.app/Contents/MacOS/Tailscale"
TAILSCALE_IP=""
if [ -x "$TAILSCALE_BIN" ]; then
  TAILSCALE_IP=$("$TAILSCALE_BIN" ip 2>/dev/null | grep -E '^100\.' | head -1 || true)
fi
...
# nohup 启动完整服务（含动态 manifest + 回测 API + SSE）
cd "$ROOT"
nohup python3 - "$PORT" "$KB" "$ROOT" > "$LOG_FILE" 2>&1 <<'PY' &
...
def build_cmd(ticker, mode):
    ...
    if mode == "simulate": cmd.append("--no-reflect")
    return cmd
def gen_manifest():
    files = []
    for p in sorted((KB / "wiki").rglob("*.md")):
...
class H(http.server.SimpleHTTPRequestHandler):
    def log_message(self, fmt, *args): pass  # 静默日志（后台模式）
    ...
    def _stream_backtest(self, ticker, mode):
        ...
        try:
            ...
            for line in proc.stdout:
                ...
                if line.startswith("OK "): okline = line
                if "report :" in line: report = line.split("report :", 1)[1].strip()
...
echo "  本机：    http://localhost:$PORT/frontend/"
if [ -n "$TAILSCALE_IP" ]; then
  echo "  iPhone：  http://$TAILSCALE_IP:$PORT/frontend/  ← 固定地址，可收藏"
else
  echo "  iPhone：  Tailscale 未连接，请先启动 Tailscale App"
fi
