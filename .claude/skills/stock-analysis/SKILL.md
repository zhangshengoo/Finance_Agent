...
6. **数据走直连、境外才代理**：国内数据源（tushare/akshare/东财 push2/新浪/雪球）一律直连走本机住宅 CN IP，仅境外 LLM（claude-max-proxy:5678→anthropic）走代理——直连同解「连通」与「风控」（境外/机房 IP 触发东财限频/断连）。直采校验固定用 tushare，勿赌裸 `ak.*`（系统代理 ProxyError + 东财 TLS 反爬）。
