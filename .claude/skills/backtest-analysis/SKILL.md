...
# Backtest Analysis (M1 单决策反思 + M2 前向净值 · A 股 only)
...
>
> **三档回测语义**：**M1 单决策反思**（最小闭环，无引擎重跑）→ **M2 前向组合模拟**（净值曲线，`--mode simulate`，前端单票已接）→ M3 历史重跑（含前瞻偏差，仅演示）。
>
> **M1 + M2 已端到端落地**：`--mode reflect`（算真实收益 + 写 5 桶记忆）/ `--mode simulate`（组合 T+1 + 逐日 mark-to-market 净值）→ `--mode export`（M1 读回教训 + 重算收益）→ 各落前端可读 `raw/analysis/backtests/cn/<ticker>-<runid>.md`（`type: backtest-report`，`mode` 区分 reflect/simulate）→ 前端「回测 / 记忆环」单元**同屏堆叠 M1（反思表 + 5 桶教训卡）+ M2（指标卡 + 净值曲线 + round-trip 成交）** → 浏览器**一键生成**经 serve.sh 仅绑 `127.0.0.1` 的 **SSE 桥** `GET /api/backtest/stream?ticker=&mode=`（M1 export / M2 simulate 双按钮，流式进度）。回测产物经 **finance-ingest 场景 F** 编 `wiki/reports/<date>-<ticker>-backtest.md` + ontology `Backtest` 节点（COVERS 边）。
...
- **前端单元已落地（M1 + M2 同屏）**：「回测 / 记忆环」挂个股详情，与「研究报告输出 / wiki」并列；上 M1 反思表 + 5 桶教训卡，下 M2 指标卡 + 净值曲线 + round-trip 成交（各取该模式最新 run）。浏览器**一键生成**经 serve.sh 仅绑 `127.0.0.1` 的 **SSE 桥** `GET /api/backtest/stream?ticker=&mode=`（M1 export / M2 simulate 双按钮，流式进度）。wiki/reports 编译 + ontology Backtest 节点已落（finance-ingest 场景 F + graph_append）。
...
- **一键生成（SSE 桥）**：浏览器单元里点「⟳ M1 反思」(mode=export) / 「📈 M2 净值模拟」(mode=simulate) → `GET /api/backtest/stream?ticker=&mode=` → serve.sh（**仅绑 127.0.0.1**，ThreadingTCPServer + 串行锁）流式跑 run_backtest，`progress`/`done` 事件 → 重载 KB 停在回测页。ticker 严格 `^\d{6}$`、mode 白名单（export|simulate）、subprocess 参数数组不走 shell。
- 回报用户：reflected N 条教训已写入 `<ticker>` 记忆桶；次日同票 `stock-analysis`（depth≥2）自动召回；前端「回测 / 记忆环」可看反思表 + 5 桶教训。

## 不做清单（明确推后）

- ❌ 港股 / 美股（→ 后续）
- ✅ M2 前向组合模拟前端：单票净值曲线 + 指标卡 + round-trip 成交已接（`renderBacktest` M2 分支）+ SSE 桥一键 `--mode simulate`。**仍推后**：多票组合净值视图、前向序列累积、reflect-on-close 教训进前端。
- ✅ wiki/reports 编译 + ontology Backtest 节点：经 `finance-ingest` 场景 F 编 `wiki/reports/<date>-<ticker>-backtest.md`（`type: report`）+ `graph_append` 建 `Backtest` 节点 + `COVERS` 边；首例 601899（+13.03%）。**仍推后**：raw-intake 溯源 handoff。
- ❌ M3 历史 walk-forward 重跑（前瞻偏差，仅演示，需显式确认 + 标注）
- ❌ 直写 wiki（须经 raw-intake → finance-ingest）
- ❌ 用 DashScope 跑对话 LLM（仅 Embedding 和 ASR）
...
- [docs/tradingagents-cn/backtest/backtest-frontend-demo.html](../../../docs/tradingagents-cn/backtest/backtest-frontend-demo.html) — 前端「回测 / 记忆环」单元设计（v5 · M1 + M2 实证版）。
- 前端接线：`frontend/index.html`（`renderBacktest()` 组合 `_btM1`/`_btM2` + `btEquitySVG()` + `genBacktest()` EventSource）、`frontend/kb-parse.js`（backtest-report 解析分支 + 星图 Backtest 标签）、`frontend/serve.sh`（动态 manifest + SSE 桥 `GET /api/backtest/stream`）。
- KB 编译：`Knowledge_Wiki/.claude/skills/finance-ingest/SKILL.md` 场景 F（回测 → wiki/reports）、`docs/kb/frontend-kb-binding.md` §2d（回测绑定契约）、`ontology/schema.yaml`（Backtest 节点 + COVERS/EVALUATES）。
